...
setwd("C:/Box Sync/Box Sync/Pdyn CEA FP Manuscript/CL8 Scripts and Functions Used R MATLAB PYTHON/")
library(R.matlab)
library(splines)
library(Matrix)
library(splines2)
...
data <- readMat('to_rA.mat')
data.ref<-data$noiseA
data.sig<-data$calciumA
...
#rm(list=ls()[! ls() %in% c("f01A","RSr","RSs")])


#Save and clear
writeMat("JOINT_A.mat", JointOut = DT.r)
...
data2 <- readMat('to_rB.mat')
data2.refB<-data2$noiseB
data2.sigB<-data2$calciumB
...
#Save and clear
writeMat("JOINT_B.mat", JointOutB = DT1.r)
