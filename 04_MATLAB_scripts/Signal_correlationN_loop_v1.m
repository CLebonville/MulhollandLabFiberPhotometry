...
%Gather length for "N"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%If running apart from df_f_compare_motion
Frequency = 1017.25262451172;
TimeFromStart = 15;% if you need to cut off any time (in seconds) from the beginning, change it here.
TimeFromEnd = 15;%if you need to cut off any time from the end, change it here.
Stop = length(data.streams.(stream05).data)-TimeFromEnd*Frequency;
Start = TimeFromStart*Frequency; 

%% Original line read: TTLa = double(data.streams.Wat1.data(round(Start):round(Stop)));
TTL = double(data.streams.(streamTTL).data(round(Start):round(Stop)));
%TTLa = double(data.streams.TTLb.data(round(Start):round(Stop))); %Use to
%switch to channel B

TTL=TTL-mode(TTL); %shifts TTLs to a '0' for baseline
TTL=(abs(TTL))*8; %makes the TTLs always in the positive direction

Time = linspace(1/Frequency, length(TTL)/Frequency, length(TTL));
raw_405 = double(data.streams.(stream05).data(round(Start):round(Stop)));
raw_470 = double(data.streams.(stream70).data(round(Start):round(Stop)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

datpoints = width(raw_405);
datpoint = [{strcat(Folders(i).name,c)}, datpoints];
CompiledDataPoints(d,:) =  datpoint;
...
FN7=[Results, 'NumberofDataPoints.xlsx'];
...
writetable(CompiledDataPoints,FN7); 
